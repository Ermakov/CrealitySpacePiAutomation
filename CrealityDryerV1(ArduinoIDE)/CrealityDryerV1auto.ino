#include <Wire.h> //свернуть все ctrl+k ctrl+0  развернуть/свернуть ctrl+shift+ ] [
#define Ttimeout    9     // minutes
#define HumMAX      30    // humidity to turn on
#define TimeCheckH  30000 // Check Humidity milliseconds (30 sec)
#define TimeCheckHs 10000  // Check Humidity in drying cycle  milliseconds (5 sec)
#define ContDryOn   20    // 
#define TimeDry     4     // hours in HEX
#define DeltaTemp   5     // С
byte StepDrying = 0;
unsigned long vTCH = TimeCheckH;
...
#define i2cSTATUS        0x27 // ON OFF ERROR (0-off; 1-on; 2-error/off; 3-error/on; 7-PowerOn)
                              //             B00000000 B00000001 B00000010 B00000011 B10000000 
...
void CmdSet(byte vCMD, byte vCMDdata = 0) {
  CMDstep = 1;
  CMD = vCMD;
  CMDdata = vCMDdata;
}
...
void I2C_RxHandler(int numBytes) {
  ...
  if (CMD == 0) {
    if (numBytes >= 2) {
      CmdSet(BufferHard[0],BufferHard[1]);
    }
    else {
      CmdSet(BufferHard[0]);
    }
...
void ScanPressKey() {
  ...
  if ((b1st+b2st+b3st+b4st) > 0)  {RefreshBuffer = true;}
}
...
bool ReadBufferSoft() {
    ...
    if (i2cStatus == i2cSTOP) {
      ...
      if (BufferSoft[0] == i2cSoftADDR) {
        ...
        if ((BufferSoft[2]+BufferSoft[3]+BufferSoft[4]) > 0) {
          cStat |= B10000000;
        }
        else {
          cStat &= B00000010;
          ...
          Serial.print(" StepDry: "); Serial.print(StepDrying);
          Serial.print(" Stat: "); Serial.print(cStat, HEX); 
          Serial.print(" min: "); Serial.print(ctM);
          Serial.print("\r\n");
        #endif
      }
...
void CheckHumidity() {
  static unsigned long mSold, mContDryOn;
  if ((millis()-mSold) > vTCH) {
    RefreshBuffer = true;
    mSold = millis();
    if ((cStat&B10000000) == 0) {return;} //power on/off
    if ((cPV_RH > HumMAX) & (CMD == 0)) {
      if (StepDrying == 0) {
        StepDrying = 1;
        mContDryOn = 0;
        vTCH = TimeCheckHs;
    ...
    if (StepDrying == 1) {
      if ((cPV_C+DeltaTemp) >= cSV) {
        StepDrying = 2;
      }
      else if (mContDryOn < ContDryOn) {
        if ((cStat&B00000001) == 0)  {
          CmdSet(i2cSetTime, 0x01); // settime 1 hour
        }
        else if (((60-ctM)>=Ttimeout)&(ctH==0)) {
          mContDryOn++;
          CmdSet(i2cSetTime, 0x00); // settime 0 hour (off)
        }
      }
      else {
        StepDrying = 2;
    ...
    else if (StepDrying == 2) {
      vTCH = TimeCheckH;
      CmdSet(i2cSetTime, TimeDry);
      StepDrying = 0;
    }
  }
}
